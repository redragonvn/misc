...
#
# Fixed the stupid code to distinguish between Anonymous and Authenticated user
# --rd

isAnon = mtool.isAnonymousUser()

comment_creator = req.get('Creator', None)
if isAnon and comment_creator:
    # Get entered anonymous name
    comment_creator = comment_creator + " (Anonymous)"
else:
...
target = '%s?portal_status_message=%s#%s' % (view, portal_status_message, anchor)
